// Reproduce the "settle & withdraw while position open" bug and verify the recovery:
// open -> undelegate WHILE OPEN (strands position on L1) -> re-delegate + close (the fix).
// Run: node scripts/test-settle-fix.cjs
...
(async () => {
  ...
  await send(base, admin, [
    ...
    SystemProgram.transfer({ fromPubkey: admin.publicKey, toPubkey: sk.publicKey, lamports: 0.05 * LAMPORTS_PER_SOL }),
  ], "fund");
  ...
  await send(base, owner, [
    ...
    await prog.methods.initPosition().accounts({ owner: owner.publicKey, market, position: pos }).instruction(),
  ...
  await send(er, sk, [await erp.methods.openPosition({ long: {} }, new anchor.BN(100_000_000), 5).accounts(erAcc).instruction()], "OPEN (session key)");

  // --- reproduce the bug: undelegate WHILE the position is open ---
  ...
  for (let i=0;i<20;i++){ const b=await base.getAccountInfo(ub); if (b && b.owner.equals(PID)) break; await new Promise(r=>setTimeout(r,1500)); }
  const onL1 = await prog.account.position.fetch(pos);
  console.log("   position stranded on L1, status:", Object.keys(onL1.status)[0], "(this is the bug state)");

  // --- THE FIX: re-delegate, then close ---
  ...
  await send(er, sk, [await erp.methods.closePosition().accounts(erAcc).instruction()], "CLOSE after recovery (session key)");
  const closed = await erp.account.position.fetch(pos);
  console.log("   position status:", Object.keys(closed.status)[0]);
  console.log("\nSETTLE-WHILE-OPEN BUG RECOVERED ✓  (re-delegate + close works)");
