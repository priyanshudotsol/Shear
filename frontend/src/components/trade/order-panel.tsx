    ...
    <div className="rounded-2xl border border-border bg-card/60 p-4">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-lg bg-secondary/40 p-1">
        <button
          ...
          className={cn(
            ...
            side === "long" ? "bg-up/15 text-up ring-1 ring-up/40" : "text-muted-foreground hover:text-foreground"
          )}
        >
          Long {active.base}-{active.quote}
        </button>
        <button
          onClick={() => setSide("short")}
          className={cn(
            "rounded-md py-2 text-sm font-semibold transition-colors",
            side === "short" ? "bg-down/15 text-down ring-1 ring-down/40" : "text-muted-foreground hover:text-foreground"
          )}
        >
          Short {active.base}-{active.quote}
        </button>
      </div>
      ...
      <div className="mt-4">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="font-medium text-muted-foreground">Collateral (USDC)</span>
          <button
            className="font-mono text-muted-foreground hover:text-foreground"
            onClick={() => setCollateral(String(Math.floor(freeCollateral)))}
          >
            Free: {fmtUsd(freeCollateral)}
          </button>
        </div>
        <div className="relative">
          <Input
            ...
            className="h-11 pr-16 font-mono text-base"
          />
          <button
            onClick={() => setCollateral(String(Math.floor(freeCollateral)))}
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded bg-secondary px-2 py-1 text-xs font-medium hover:bg-accent"
          >
            MAX
          </button>
        </div>
        ...
        <div className="mb-2 flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              disabled={leverage <= 1}
              className="grid h-6 w-6 place-items-center rounded-md bg-secondary text-muted-foreground transition-colors hover:text-foreground disabled:opacity-40"
      ...
      {/* preview */}
      <div className="mt-4 space-y-1.5 rounded-lg border border-border/70 bg-secondary/20 p-3 text-sm">
        <Row label="Notional" value={fmtUsd(n)} />
        <Row label="Entry ratio" value={fmtRatio(ratio)} />
        <Row
          label="Liquidation ratio"
          value={fmtRatio(liqRatio)}
          valueClass={side === "long" ? "text-down" : "text-down"}
        />
        ...
        <Row label="Pool capacity" value={fmtUsd(availNotional, 0)} valueClass={exceedsPool ? "text-down" : undefined} sub="max notional" />
      </div>
          ...
          <>
            <Button
              ...
              className={cn(
                ...
                side === "long" ? "bg-up/80 text-up-foreground hover:bg-up" : "bg-down/80 text-down-foreground hover:bg-down"
              )}
            >
...
function Row({ label, value, sub, valueClass }: { label: string; value: string; sub?: string; valueClass?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={cn("font-mono", valueClass)}>
        {value}
